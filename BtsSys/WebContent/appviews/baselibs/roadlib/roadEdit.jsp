...
$(function(){
	...
	$("#remark").ligerTextBox({width : 600 ,height:50 });
	 ...
	 $("#testtime").ligerDateEditor({ label: '', format: "yyyy-MM-dd", labelAlign: 'right',initValue: '${roadLib.testTimeStr}',width : 200});
});
   ...
   //道路属性
	...
	$.getJSON(typeURL2, 
		function(data){
			...
			sl2.selectValue('${roadLib.covertype}');
		}
	);
...
<!-- 标题 -->
	...
	<div class="main_title_2">
              <p class="main_title_p"><img src="${ctx}/layouts/image/ico_arrow.gif"></img>编辑道路库信息</p>
	</div>
	<div class="content">
	<form method="post" name="form1" id="form1" action="${ctx}/schooljson/editRoadLib.action">
		<table cellpadding="0" cellspacing="0" class="tab_1" >
            <tr>
                ...
                <td width="300px">
					...
					<input name="roadLib.Id" id="id" type="hidden" value="${roadLib.id}"/>
				</td>
				<th width="150px"><span style="color: red;">*</span>线路属性:</th>
                <td width="300px">
            ...
            <tr>
				...
				<td>
					<input name="roadLib.speed" type="text" id="speed" class="{required:true}" value="${roadLib.speed}"/> 
				</td>
            </tr>
             <tr>
                <th><span style="color: red;">*</span>辖区内线路里程(km)：</th>
				<td>
					<input name="roadLib.lineMile" id="lineMile" type="text" class="{required:true,number:true}" value="${roadLib.lineMile}"/>
				</td>
				<th><span style="color: red;">*</span>辖区内隧道里程(km):</th>
                <td>
					<input name="roadLib.tunnelMile" type="text" id="tunnelMile" class="{required:true,number:true}" value="${roadLib.tunnelMile}"/> 
				</td>           
			</tr>
			<tr>
				<th><span style="color: red;">*</span>辖区内平原段里程(km):</th>
                <td>
					<input name="roadLib.plainMile" type="text" id="plainMile" class="{required:true,number:true}" value="${roadLib.plainMile}"/> 
				</td>
				<th><span style="color: red;">*</span>辖区内山区段里程(km)：</th>
				<td>
					<input name="roadLib.mountainMile" type="text" id="mountainMile" class="{required:true,number:true}" value="${roadLib.mountainMile}"/> 
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>辖区内车站（收费站或服务区）数量：</th>
				<td>
			        <input name="roadLib.stationNum" type="text" id="stationNum" class="{required:true,number:true}" value="${roadLib.stationNum}"/> 
				</td>
            ...
            <tr>
				<th>辖区内1X覆盖里程（km）：</th>
				<td>
					<input name="roadLib.coverMile1x" type="text" id="coverMile1x" class="{number:true)" value="${roadLib.coverMile1x}"/>
				</td>
				<th> 辖区内1X里程覆盖率（%）：</th>
				<td>
					<input name="roadLib.coverRate1x" type="text" id="coverRate1x" class="{number:true}" value="${roadLib.coverRate1x}"/>
				</td>
			</tr>
				<tr>
				...
				<th>里程掉话比（km）：</th>
				<td>
					<input name="roadLib.dropcallMile" type="text" id="dropcallMile" class="{number:true}" value="${roadLib.dropcallMile}"/>
				</td>
				<th>辖区内DO覆盖里程（km）：</th>
				<td>
					<input name="roadLib.coverMileDo" type="text" id="coverMileDo" class="{number:true}" value="${roadLib.coverMileDo}"/>
				</td>
            </tr>
            <tr>		
				<th>辖区内DO里程覆盖率（%）：</th>
				<td>
					<input name="roadLib.coverRateDo" type="text" id="coverRateDo" class="{number:true}" value="${roadLib.coverRateDo}"/>
				</td>
				<th>有覆盖区域的下行吞吐率（%）：</th>
				<td>
					<input name="roadLib.downTpRate" type="text" id="downTpRate" class="{number:true}" value="${roadLib.downTpRate}"/>
				</td>
            </tr>
             <tr>
				
				<th>有覆盖区域下行吞吐率优良比(>=300kbps)：</th>
				<td>
					<input name="roadLib.downTpgoodRate" type="text" id="downTpgoodRate" class="{number:true}" value="${roadLib.downTpgoodRate}"/>
				</td>
				<th>辖区内已覆盖车站（收费站或服务区）数量：</th>
				<td>
					<input name="roadLib.coverStationNum" type="text" id="coverStationNum" class="{number:true}" value="${roadLib.coverStationNum}"/>
				</td>
            ...
            <tr>
				...
				<td>
					<input name="roadLib.coverStationRate" type="text" id="coverStationRate" class="{number:true}" value="${roadLib.coverStationRate}"/>
				</td>
				<th>此次覆盖数据路测时间：</th>
				<td>
					<input name="roadLib.testtime" type="text" id="testtime" value="${roadLib.testtime}"/>
				</td>
            ...
            <tr>
				...
				<th> 测试人员：</th>
				<td>
					<input name="roadLib.testuser" type="text" id="testuser" value="${roadLib.testuser}"/>
				</td>
