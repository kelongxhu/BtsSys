...
$(function(){
	...
	$.post("${ctx}/schooljson/initCountryTree.action", function(
			...
			treeCombox.selectValue('${tunnelLib.countryId}');
	});
	...
	var treeCombox2;
     ...
     $.post("${ctx}/schooljson/roadTree.action", function(
		...
		treeCombox2.selectValue('${tunnelLib.roadId}');
});	
...
<!-- 标题 -->
	...
	<div class="main_title_2">
              <p class="main_title_p"><img src="${ctx}/layouts/image/ico_arrow.gif"></img>编辑隧道库信息</p>
	</div>
	<div class="content">
	<form method="post" name="form1" id="form1" action="${ctx}/schooljson/editTunnelLib.action">
		<table cellpadding="0" cellspacing="0" class="tab_1" >
            <tr>
                ...
                <td width="300px">
					...
					<input name="tunnelLib.id" id="id" type="hidden" value="${tunnelLib.id}"/>
				</td>
             ...
             <tr>
				...
				<td>	
					<input name="tunnelLib.name" id="name" type="text" class="{required:true}" value="${tunnelLib.name}"/>
				</td>
				 <th><span style="color: red;">*</span>隧道长度：</th>
				<td>	
					<input name="tunnelLib.tunnellength" id="tunnellength" type="text" class="{required:true}" value="${tunnelLib.tunnellength}"/>
				</td>
			</tr>
			<tr>
                <th><span style="color: red;">*</span>方向：</th>
				<td>	
					<input name="tunnelLib.direction" id="direction" type="text" class="{required:true}" value="${tunnelLib.direction}"/>
				</td>
				 <th><span style="color: red;">*</span>经度：</th>
				<td>	
					<input name="tunnelLib.longitude" id="longitude" type="text" class="{required:true,number:true}" value="${tunnelLib.longitude}"/>
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>纬度:</th>
                <td>
					<input name="tunnelLib.latitude" type="text" id="latitude" class="{required:true,number:true}" value="${tunnelLib.latitude}"/> 
				</td>
				<th><span style="color: red;">*</span>海拔高度:</th>
                <td>
					<input name="tunnelLib.height" type="text" id="height" class="{required:true,number:true}" value="${tunnelLib.height}"/> 
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>覆盖设备类型:</th>
                <td>
					<input name="tunnelLib.covgtype" type="text" id="covgtype" class="{required:true}" value="${tunnelLib.covgtype}"/> 
				</td>
				<th><span style="color: red;">*</span>设备安装位置:</th>
                <td>
					<input name="tunnelLib.installadress" type="text" id="installadress" class="{required:true}" value="${tunnelLib.installadress}"/> 
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>产权:</th>
                <td>
					<input name="tunnelLib.rights" type="text" id="rights" class="{required:true}" value="${tunnelLib.rights}"/> 
				</td>
				<th><span style="color: red;">*</span>是否共建共享:</th>
                <td>
					<input name="tunnelLib.togbs" type="text" id="togbs" class="{required:true}" value="${tunnelLib.togbs }"/> 
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>共享方名称:</th>
                <td>
					<input name="tunnelLib.togname" type="text" id="togname" class="{required:true}" value="${tunnelLib.togname }"/> 
				</td>
				<th><span style="color: red;">*</span>设备开通年月:</th>
                <td>
					<input name="tunnelLib.opentime" type="text" id="opentime" class="{required:true}" value="${tunnelLib.opentime}"/> 
				</td>
			</tr>
			<tr>
				<th><span style="color: red;">*</span>详细地址:</th>
                <td colspan="3">
					<input name="tunnelLib.address" type="text" id="address" class="{required:true}" value="${tunnelLib.address}"/> 
				</td>
			</tr>
			 <tr>
				<th>备注：</th>
				<td>
					<textarea name="tunnelLib.remark"  id="remark">${tunnelLib.remark}</textarea>
				</td>
            </tr>
            <tr>
			   			 <td></td>
