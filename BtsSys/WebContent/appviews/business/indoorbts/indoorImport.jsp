    ...
    <script type="text/javascript">
        $(function () {
            $("#uploadify1").uploadify({
                ...
                uploader:'${ctx}/businessjson/importIndoorInputData.action;jsessionid=${pageContext.session.id}',
                fileObjName:'file',
                fileSizeLimit:0,
                successTimeout:600,
                multi:false,
            ...
            $("#uploadify2").uploadify({
                method:'post',
                swf:'${ctx}/resources/uploadify/uploadify.swf',
                uploader:'${ctx}/businessjson/importErctsInputData.action;jsessionid=${pageContext.session.id}',
                fileObjName:'file',
                fileSizeLimit:0,
                successTimeout:600,
                multi:false,
            ...
            $("#uploadify3").uploadify({
                method:'post',
                swf:'${ctx}/resources/uploadify/uploadify.swf',
                uploader:'${ctx}/businessjson/importDryInputData.action;jsessionid=${pageContext.session.id}',
                fileObjName:'file',
                fileSizeLimit:0,
                successTimeout:600,
                multi:false,
                ...
                onUploadSuccess:function (file, data, response) {
                   data = eval('(' + data + ')');
                    if (data.result == 1) {
                        ...
                        if (error && error.length > 0) {
                            for (var i = 0; i < error.length; i++) {
                                $("#error3").append("<p>" + error[i] + "</p>");
                            }
                        }
                    } else {
                        $("#msg3").html("文件：" + file.name + "导入过程中出现异常!导入成功条数："+data.sucess);
                    }
                }
            });
        });
        ...
        function uploadifyUpload3() {
            $('#uploadify3').uploadify('upload', '*');
            $("#msg3").html("请稍等,正在导入。。。");
            $("#msg3").addClass("progressCss");
        }
    ...
    <div class="content">
        <div class="tabbable tabs-left">
            <ul class="nav nav-tabs nav-stacked" id="mytab">
                <li class="active"><a href="#tab1" data-toggle="tab">室分导入</a>
                </li>
                <li><a href="#tab2" data-toggle="tab">直放导入</a></li>
                <li><a href="#tab3" data-toggle="tab">干放导入</a></li>
            </ul>
            <div class="tab-content">
                <div class="tab-pane active" id="tab1">
                    ...
                    <span style="font-weight: bold;color: #EA5200;">
                        请先导出待录入数据模板，按照录入模板填写对应数据，导入室分手工数据!
                    </span>
                            </div>
                        </div>
                ...
                <div class="tab-pane" id="tab3">
                    ...
                    <span style="font-weight: bold;color: #EA5200;">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input id="uploadify3" name="uploadify3" type="file" multiple="true">
                                <div id="msg3"></div>
                                <div id="error3" class="result"></div>
                            </div>
                        </div>
                    </form>
                    <div class="form-actions">
                        <button class="btn btn-info" type="button" onclick="uploadifyUpload3();"><i class="icon-ok icon-white"></i>上传</button>
                        <button class="btn btn-info" type="reset" onclick="back();"> <i class="icon-repeat"></i>返回</button>
                    </div>
                </div>
            </div>
