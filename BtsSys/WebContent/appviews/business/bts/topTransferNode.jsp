...
<head>
    ...
    <script type="text/javascript">
        ...
        function transfer(url) {
            gridObj = $("#maingrid").ligerGrid({
                columns: [
                    {display: '传输节点名称', name: 'name', width: 400, align: 'center'},
                    {display: '本地网', name: 'cityName', width: 80, align: 'center'},
        ...
        function toSearch() {
            ...
            var url;
            if (type == 2) {
                url = encodeURI("${ctx}/businessjson/queryTransferList?name=" + name);
                transfer(url);
            } else if (type == 3) {
        ...
        function select() {
            ...
            if (rows.length == 0) {
                $.ligerDialog.alert('请选择一个传输上游拓扑站点!');
                return;
            } else if (rows.length > 1) {
                $.ligerDialog.alert('只能选择一个传输上游拓扑站点!');
                return;
            }
            var tranUpsitenameVal = parent.window.document.getElementById("tranUpsitenameVal");
            var tranUpsitename = parent.window.document.getElementById("tranUpsitename");
            tranUpsitenameVal.value = name;
            tranUpsitename.value = str;
            parent.$.ligerDialog.close();
            parent.$(".l-dialog,.l-window-mask").remove();
        }
    </script>
